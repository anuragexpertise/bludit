...
; ?> method="post">
    ...
    <input type="hidden" id="tokenCSRF" name="tokenCSRF" value="<?php echo $tokenCSRF; ?>">
    ...
    <div x-data="content">
        ...
        <select name="filename"
            style="padding:10px;background:#fafafa;border:solid 1px #ddd;width:100%;margin:10px 0;border-radius:5px;">
        ...
        <div class="add-input">
            ...
            <br>
            <button @click.prevent="inputList.unshift({ 'label': title, 'value': '', 'type': select });count++"
                x-html="buttonAdd"></button>
        ...
        <div style="margin:10px 0;" id="sortable">
            <template x-for="(input,index) in inputList" :key="index">
                <div class="form-input">
                    ...
                    <input type="hidden" name="multi-field-label[]" :value="input.label.replace(/ /g,'-')"
                        placeholder="label">
                    <select name="multi-field-type[]">
                        <template x-for="(selector,index) in selectList">
                            <option :value="selector" :selected="input.type==selector" x-text="selectListLang[index]">
                            </option>
                        </template>
                    </select>
                    <button @click.prevent="if(confirm(sureQuestion)){inputList.splice(index, 1);}"
                        x-html="remove"></button>
        ...
        <button style="display:inline-flex;align-items:center;justify-content:center;" class="sortable order"
            x-html="sortName" @click.prevent="sortable"></button>
...
<script>
    const content = {
        ...
        selectList: ["text", "textarea", "wysywig", "color", "date", "foto", "link", "checkbox", "dropdown", "video"],
        selectListLang: ["text", "textarea", "wysywig", "color", "date", "foto", "link", "checkbox", "dropdown", "video"],
        inputList: <?php
...
<script>
    function handleAjaxUpload(event, type) {
        ...
        formData.append('tokenCSRF', document.querySelector('input[name="tokenCSRF"]').value);
        formData.append(type, file);

        fetch('<?php echo HTML_PATH_ADMIN_ROOT . "plugin/mediauploader/ajax/upload/"; ?>' + type, {
            method: 'POST',
            body: formData
        })
            .then(response => response.json())
            .then(data => {
                console.log(`Upload response (${type}):`, data);
                if (data.success) {
                    alert(`${type} uploaded successfully: ${data.path}`);
                } else {
            ...
            .catch(error => {
                ...
                alert(`An error occurred during ${type} upload`);
            });
    }
</script>